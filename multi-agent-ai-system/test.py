...
def print_result(stream, token_usage, llm_call):
    ...
    print()
    pprint(stream.output['course_state'], indent=2)
    print('\nToken Usage')
...
first_msg = AIMessage('May I know what is your preferred course and uni?')
...
while not stop_chat:
    ...
    stream = graph.stream_events({'messages': messages, 'course_state': {'max_indecision_count': 1}}, config=config, version='v3')
